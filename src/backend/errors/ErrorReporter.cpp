...
namespace forge::errors {
    ...
    void ErrorReporter::reportBorrowError(BorrowError::Kind kind, std::string message, SourceLocation location) {
        auto error = std::make_unique<BorrowError>(kind, message, location);
        reportError(std::move(error));
    }

    void ErrorReporter::reportCodegenError(std::string message, SourceLocation location) {
        auto error = std::make_unique<CompilerError>(CompilerError::Level::Error, message, location);
        reportError(std::move(error));
    }
